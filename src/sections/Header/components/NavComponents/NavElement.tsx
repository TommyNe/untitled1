                ...
                <div>
                    {subMenuItems &&
                        subMenuItems.map(item => (
                            <li
                                key={item.link}
                                className="relative text-sm box-border caret-transparent block leading-[normal] text-left w-full md:text-center md:w-auto group/sub"
                            >
                                <Link
                                    to={item.link}
                                ...
                                >
                                    {item.label}
                                    {item.subItems && <span className="ml-1">▸</span>}
                                </Link>
                                {item.subItems && (
                                    <ul className="md:absolute md:left-full md:top-0 md:min-w-[200px] md:shadow-[rgba(0,0,0,0.2)_0px_2px_2px_0px] md:rounded md:bg-white md:opacity-0 md:invisible md:translate-x-1 md:transition-all md:duration-200 md:group-hover/sub:visible md:group-hover/sub:opacity-100 md:group-hover/sub:translate-x-0 pl-4 md:pl-0">
                                        {item.subItems.map(subItem => (
                                            <li key={subItem.link} className="text-sm">
                                                <Link
                                                    to={subItem.link}
                                                    className="text-zinc-600 block py-2.5 px-2 hover:text-teal-700"
                                                >
                                                    {subItem.label}
                                                </Link>
                                            </li>
