...
export function NewsHomeList() {
    const pageSize = 5
    const [page, setPage] = useState(1)

    const { data, isLoading, isError } = useQuery<{ items: ContentItem[]; totalItems: number }>({
        queryKey: ['posts', page],
        queryFn: async () => {
            const res = await axios.get('https://etwah.de/api/posts', {
                headers: { Accept: 'application/ld+json' },
                params: { page, itemsPerPage: pageSize }
            })
            const items = (res.data['hydra:member'] as ContentItem[]) || []
            const totalItems = Number(res.data['hydra:totalItems'] ?? items.length)
            return { items, totalItems }
        },
        placeholderData: prev => prev
    })

    const posts = (data?.items || []).filter(item => item.vis)

    const { items, totalPages } = useMemo(() => {
        const total = data?.totalItems ?? posts.length
        const totalPages = Math.max(1, Math.ceil(total / pageSize))
        return { items: posts, totalPages }
    }, [data?.totalItems, posts])
    ...
    const renderList = () => {
        if (isLoading) return <div>Lade Beiträge...</div>
        if (isError) return <div>Beiträge konnten nicht geladen werden.</div>
        return (
